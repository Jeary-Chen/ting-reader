...
export const getCoverUrl = (url?: string, libraryId?: string, bookId?: string) => {
  ...
  if (!url) return getRuntimeAssetUrl('/placeholder-cover.png');
  ...
  if (url.startsWith('http') && url.includes('#referer=')) {
    ...
    let coverUrl = `${getRuntimeUrl('/api/proxy/cover', baseUrl)}?path=${encodeURIComponent(url)}`;
    if (libraryId) {
  ...
  if (libraryId) {
    let coverUrl = `${getRuntimeUrl('/api/proxy/cover', baseUrl)}?path=${encodeURIComponent(url)}&library_id=${libraryId}`;
  ...
  if (url.startsWith('/')) {
      return getRuntimeUrl(url, baseUrl);
  }
